...
public class Board
{
    ...
    public static int allcell;
    private GameSettings m_settings;
    private GameObject BottomCellPrefab;

    private List<Cell> bottomCells = new List<Cell>();
    ...
    public void CreateBottomCell()
    {
        BottomCellPrefab = Resources.Load<GameObject>("prefabs/cellbottom");
        float bottomSpacing = 1.0f; 
        Vector3 origin = new Vector3(-boardSizeX * 0.5f + 0.5f - 2.5f, -boardSizeY * 0.5f - bottomSpacing, 0f);

        bottomCells.Clear(); 

        for (int x = 0; x < boardSizeX + 5; x++)
        {
            Vector3 bottomPos = origin + new Vector3(x, 0, 0); 
            GameObject bottomCell = GameObject.Instantiate(BottomCellPrefab, bottomPos, Quaternion.identity, m_root);
            bottomCells.Add(bottomCell.GetComponent<Cell>());
            bottomCell.GetComponent<Cell>().isbottomcell = true;
        }
    }

    
    public Cell GetAvailableBottomCell()
    {
    ...
    {
    return boardSizeY*boardSizeX;
    }
    public void SetHasMadeMove()
    {
        hasMadeMove = true;
    }
    ...
    public void CheckAndClearMatchingItems()
    {
        Dictionary<string, List<Cell>> itemGroups = new Dictionary<string, List<Cell>>();

        // Gom nhóm item theo tên GameObject
        foreach (Cell cell in bottomCells)
        {
            if (cell.Item != null && cell.Item.View != null)
            {
                string itemName = cell.Item.View.name;

                if (!itemGroups.ContainsKey(itemName))
                {
                    itemGroups[itemName] = new List<Cell>();
                }
                itemGroups[itemName].Add(cell);
            }
        }

        bool itemsCleared = false;

        // Kiểm tra nhóm nào có ít nhất 3 item, thì xóa chúng
        ...
        // Nếu có item bị xóa, sắp xếp lại và kiểm tra tiếp
        if (itemsCleared)
        {
            ShiftBottomCellsDown();
            CheckAndClearMatchingItems();
            BoardController.b = BoardController.b - 3;
            Debug.Log(BoardController.b);
        }
    }
    public void ShiftBottomCellsDown()
    {
        List<Item> remainingItems = new List<Item>();

        // Lưu lại tất cả item còn lại
        foreach (Cell cell in bottomCells)
        {
            if (cell.Item != null)
            {
                remainingItems.Add(cell.Item);
                cell.Free(); // Xóa item khỏi ô cũ
            }
        }

        // Gán lại item từ trên xuống dưới
        for (int i = 0; i < remainingItems.Count; i++)
        {
            bottomCells[i].Assign(remainingItems[i]);
            bottomCells[i].ApplyItemMoveToPosition();
        }
    }
    ...
    public List<Cell> GetBottomCells()
    {
        List<Cell> bottomCells = new List<Cell>();
        for (int x = 0; x < boardSizeX; x++)
        {
            bottomCells.Add(m_cells[x, 0]); // Lấy các ô ở hàng dưới cùng (y = 0)
        }
        return bottomCells;
    }
    ...
    public void OnCellSelected(Cell selectedCell)
    {
        ...
        Item item = selectedCell.Item;
        Debug.Log(selectedCell.isbottomcell);
        if (selectedCell.isbottomcell == false)
        {
            selectedCell.Free();
        }
        

        // Nếu ô được chọn là ô thuộc hàng bottom cell, tạo vật phẩm mới
        if (bottomCells.Contains(selectedCell))
        {
            ...
            selectedCell.Assign(newItem);
            selectedCell.ApplyItemPosition(true);
        }

        // Dịch chuyển các ô phía trên xuống
        ShiftDownItems();
    }
    ...
    internal void ShiftDownItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            ...
            // Điền các ô trống ở hàng trên cùng bằng các vật phẩm mới
            for (int y = boardSizeY - shifts; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x, y];
                NormalItem newItem = new NormalItem();
                newItem.SetType(Utils.GetRandomNormalType());
